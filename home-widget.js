(function () {
  ...
  window.SF_CATALOG_URL = window.SF_CATALOG_URL || "https://simplefrancais.getcourse.ru/сalatogue";
  ...
  style.textContent = `
    ...
    .sf-search { width:min(420px, 42vw); min-height:46px; padding:0 16px; border:1px solid var(--sf-line); border-radius:8px; display:flex; align-items:center; gap:10px; color:var(--sf-muted); background:#fff; box-shadow:0 8px 22px rgba(19,35,68,.04); }
    .sf-search:before { content:"⌕"; color:#2f4ccf; font-size:19px; line-height:1; }
    ...
    .sf-link { display:inline-flex; align-items:center; gap:7px; color:#344bd8 !important; text-decoration:none !important; font-weight:800; font-size:14px; }
    .sf-link:after { content:"›"; font-size:19px; line-height:1; transform:translateY(-1px); }
    ...
    .sf-hero-badges { display:flex; flex-wrap:wrap; align-items:center; gap:10px; }
    ...
    .sf-streak { display:inline-flex; width:max-content; align-items:center; min-height:24px; padding:0 10px; border-radius:7px; background:rgba(255,255,255,.12); color:#fff; font-size:12px; font-weight:850; }
    ...
    .sf-primary { display:inline-flex; align-items:center; justify-content:center; min-height:46px; padding:0 24px; border:0; border-radius:8px; background:#5b4eea; color:#fff !important; text-decoration:none !important; font-weight:850; cursor:pointer; box-shadow:0 12px 28px rgba(91,78,234,.28); transition:transform .18s ease, box-shadow .18s ease; }
    .sf-primary:hover { transform:translateY(-1px); box-shadow:0 16px 34px rgba(91,78,234,.36); }
    ...
    .sf-recent-card { position:relative; display:grid; grid-template-columns:46px minmax(0,1fr); gap:12px; align-items:center; flex:0 0 min(285px, 82vw); scroll-snap-align:start; padding:13px; transition:transform .18s ease, box-shadow .18s ease; }
    .sf-recent-card:hover { transform:translateY(-2px); box-shadow:0 16px 34px rgba(17,34,63,.18); }
    .sf-recent-menu { position:absolute; top:9px; right:10px; color:rgba(255,255,255,.7); font-weight:900; letter-spacing:1px; }
    a.sf-recent-card { color:#fff !important; text-decoration:none !important; }
    ...
    .sf-rec { min-height:168px; border:1px solid var(--sf-line); border-top:3px solid var(--accent,#5b5ff0); border-radius:8px; background:#fff; padding:13px 14px 14px; box-shadow:0 10px 25px rgba(17,34,63,.06); display:flex; flex-direction:column; justify-content:space-between; color:var(--sf-ink) !important; text-decoration:none !important; transition:transform .18s ease, box-shadow .18s ease, border-color .18s ease, background .18s ease; }
    ...
    .sf-rec-icon { width:42px; height:42px; border-radius:9px; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg, rgba(255,255,255,.92), rgba(238,243,255,.96)); color:var(--accent,#5b5ff0); font-size:23px; font-weight:900; box-shadow:inset 0 0 0 1px rgba(56,94,234,.08); }
    ...
    .sf-box-title { display:flex; align-items:center; gap:8px; }
    .sf-news { position:relative; display:grid; grid-template-columns:42px minmax(0,1fr) 18px; align-items:center; gap:12px; min-height:68px; padding:10px; border:1px solid #e9eef7; border-radius:8px; margin-bottom:9px; transition:transform .18s ease, border-color .18s ease, box-shadow .18s ease; }
    a.sf-news:hover { transform:translateY(-2px); border-color:#cbd8fb; box-shadow:0 12px 25px rgba(17,34,63,.08); }
    .sf-news:last-child { margin-bottom:0; }
    .sf-news b { display:block; font-size:13px; color:var(--sf-ink); }
    .sf-news small { display:block; color:var(--sf-muted); line-height:1.35; }
    .sf-news-arrow { color:#3146d3; font-weight:950; font-size:20px; line-height:1; }
    ...
    .sf-action { min-height:42px; border:1px solid #e0e7f2; border-radius:8px; background:#fff; color:var(--sf-ink) !important; text-decoration:none !important; display:flex; align-items:center; justify-content:space-between; padding:0 12px; text-align:left; font-size:13px; font-weight:850; transition:transform .18s ease, border-color .18s ease, background .18s ease; }
    .sf-action:after { content:"›"; color:#3146d3; font-size:18px; font-weight:950; }
    .sf-action:hover { transform:translateY(-1px); border-color:#cbd8fb; background:#fbfcff; }
  ...
  function userName() {
    const raw = String(
      window.accountUserName ||
      (window.accountUser && (window.accountUser.firstName || window.accountUser.name)) ||
      (window.gcUser && (window.gcUser.first_name || window.gcUser.firstName || window.gcUser.name)) ||
      (window.userInfo && (window.userInfo.first_name || window.userInfo.firstName || window.userInfo.name)) ||
      text(".gc-account-user-name") ||
      text(".user-profile-name") ||
      text(".xdget-user-name") ||
      root.dataset.userName ||
      ""
    ).trim();
    const clean = raw.replace(/\s+/g, " ");
    return clean ? clean.split(" ")[0] : "студент";
  }
  ...
  function visualIcon(item) {
    const raw = String((item && (item.title + " " + item.type + " " + item.section + " " + item.level)) || "").toLowerCase();
    if (raw.includes("ауд") || raw.includes("delf")) return "🎧";
    if (raw.includes("фонет")) return "📗";
    if (raw.includes("чтен")) return "📖";
    if (raw.includes("дикт")) return "✎";
    if (raw.includes("марафон") || raw.includes("mini") || raw.includes("мини")) return "⏱";
    if (raw.includes("фраз") || raw.includes("турист")) return "🧳";
    if (raw.includes("глаг")) return "✨";
    return item.icon || item.level || "FR";
  }

  function topLink(label, url) {
    return url ? '<a class="sf-link" href="' + escapeHtml(url) + '">' + escapeHtml(label) + '</a>' : '';
  }

  function actionLink(label, url) {
    return url ? '<a class="sf-action" href="' + escapeHtml(url) + '">' + escapeHtml(label) + '</a>' : '';
  }

  function cardLinkStart(className, url, attrs) {
    return url && url !== "#" ? '<a class="' + className + '" href="' + escapeHtml(url) + '"' + (attrs || "") + '>' : '<div class="' + className + '"' + (attrs || "") + '>';
  }
  ...
  function renderHero(activity, stat) {
    if (!activity) {
      return '<section class="sf-section">' +
        ...
        '<article class="sf-hero"><div class="sf-hero-content">' +
          '<div><div class="sf-hero-badges"><span class="sf-pill">FR</span><span class="sf-streak">Новый старт</span></div><h3>Выберите свой уровень</h3><p>Когда вы откроете первый урок, здесь появится продолжение обучения.</p></div>' +
    ...
    return '<section class="sf-section">' +
      '<div class="sf-heading"><h2>Продолжить обучение</h2></div>' +
      '<article class="sf-hero"><div class="sf-hero-content">' +
        '<div><div class="sf-hero-badges"><span class="sf-pill">' + escapeHtml(activity.level || "FR") + '</span><span class="sf-streak">🔥 Вернуться к уроку</span></div>' +
  ...
  function renderRecent(courses) {
    const items = (courses || []).slice(0, 5).map((course) => {
      const url = course.last_lesson_url || course.url || "";
      return cardLinkStart("sf-recent-card", url) +
        '<div class="sf-icon" style="--accent:' + escapeHtml(course.accent) + '">' + escapeHtml(course.icon || course.level || "FR") + '</div>' +
        '<span class="sf-recent-menu">⋮</span><div><h3>' + escapeHtml(course.title) + '</h3><p>' + escapeHtml(course.level || "Курс") + '</p>' +
  ...
  function renderLevels(levels) {
    const cards = levels.map((level) => {
      return '<article class="sf-level-card" style="--accent:' + escapeHtml(level.accent) + '">' +
        ...
        (level.url ? '<a class="sf-open" href="' + escapeHtml(level.url) + '">Открыть</a>' : '') +
      '</article>';
    }).join("");
  ...
  function renderRecommendations(items) {
    const cards = items.slice(0, 5).map((item) => {
      ...
      return cardLinkStart("sf-rec", item.url, attrs) + '<div><div class="sf-rec-head"><span class="sf-tag">' + escapeHtml(item.level) + '</span><span class="sf-rec-icon">' + escapeHtml(visualIcon(item)) + '</span></div><h3>' + escapeHtml(item.title) + '</h3><p>' + escapeHtml(item.subtitle || item.type) + '</p></div><div class="sf-rec-foot"><p>' + (item.total_lessons ? item.total_lessons + ' уроков' : 'Скоро') + '</p><span class="sf-rec-arrow">›</span></div>' + cardLinkEnd(item.url);
    }).join("");
  ...
  function renderSide(dashboard) {
    ...
    const news = (dashboard.news || []).filter((item) => item.url && item.url !== "#").slice(0, 4).map((item) => {
      const attrs = ' style="text-decoration:none; color:inherit"';
      return cardLinkStart("sf-news", item.url, attrs) + '<div class="sf-icon" style="--accent:' + escapeHtml(item.accent || "#75d56f") + '">' + escapeHtml(item.icon || item.level || "FR") + '</div><div><span class="sf-news-time">' + escapeHtml(item.time_label || item.date || "") + '</span><b>' + escapeHtml(item.title) + '</b><small>' + escapeHtml(item.subtitle || item.level || "") + '</small></div><span class="sf-news-arrow">›</span>' + cardLinkEnd(item.url);
    }).join("");
    const usefulLinks = [
      actionLink("Задать вопрос", window.SF_HELP_URL || ""),
      actionLink("Навигатор по платформе", window.SF_NAVIGATOR_URL || ""),
      actionLink("Предложить тему курса", window.SF_SUGGEST_TOPIC_URL || "")
    ].filter(Boolean).join("");
    const catalogUrl = window.SF_CATALOG_URL || "";

    return '<aside class="sf-side">' +
      (news ? '<div class="sf-box"><h2 class="sf-box-title">🎉 Что нового?</h2>' +
        news +
      '</div>' : '') +
      '<div class="sf-box"><h2 class="sf-box-title">🏅 Мои достижения</h2>' +
        '<div class="sf-news"><div class="sf-icon">' + platform.percent + '%</div><div><b>Прогресс платформы</b><small>' + platform.opened_lessons + ' из ' + platform.total_lessons + ' уроков открыто</small></div><span class="sf-news-arrow">↗</span></div>' +
        '<div class="sf-news"><div class="sf-icon">100</div><div><b>Цель</b><small>Следующая отметка: 100 уроков</small></div><span class="sf-news-arrow">◎</span></div>' +
      '</div>' +
      (usefulLinks ? '<div class="sf-box"><h2 class="sf-box-title">💡 Полезное</h2><div class="sf-action-grid">' + usefulLinks + '</div></div>' : '') +
      (catalogUrl ? '<div class="sf-box sf-catalog-box"><h2 class="sf-box-title">📘 Каталог</h2><a class="sf-action" href="' + escapeHtml(catalogUrl) + '">Все курсы</a></div>' : '') +
    '</aside>';
  }
  ...
  function renderDashboard(dashboard) {
    ...
    root.innerHTML = '<div class="sf-shell">' +
      '<main class="sf-main">' +
        '<header class="sf-top"><div class="sf-hello"><h1>Bonjour, ' + escapeHtml(name) + '! 👋</h1><p>Продолжайте учиться и достигайте новых целей</p></div><div class="sf-search">Поиск курсов, уроков, тем...</div></header>' +
